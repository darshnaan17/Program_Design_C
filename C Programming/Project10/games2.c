...
int games_compare(const void* p, const void* q);
int main()
{
...
qsort(PS4_games, count, sizeof(struct game), games_compare);
...
int games_compare(const void* p, const void* q)
{
    if (((struct game *) p)->quant < ((struct game *) q)->quant)
        return 1;
    else if (((struct game *) p)->quant == ((struct game *) q)->quant)
        return 0;
    else
        return -1;
}
