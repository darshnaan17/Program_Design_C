...
struct request* insert_to_order(struct request* list){
...
else
{
    initial = NULL;
    current = list;
    while(current != NULL) //checking if there already is a c_id and s_id
    {
        if(strcmp(current -> courseID,c_id) == 0) // courseid comes first in the project requirements
        {
            if (strcmp(current -> studentID,s_id) == 0)
            {
                printf("This request exists already"); return list;
            }
            else if(strcmp(current -> studentID, s_id)>0)
            {
                break;
            }
        }
        else if(strcmp(current -> courseID, c_id)>0)
        {
            break;
        }
